...
from tkinter import filedialog #올에 명시되어잇지않음
...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요.",\
        ...
        initialdir = r"C:/Users/master1/Desktop/PythonWorkspace/")
        #r로 그냥 그문자 그대로 쓰겟다
    # 목록출력
    for file in files:
...
def merge_image():
    ...
    #     y_offset += img.size[1] # 높이값 만큼 더함

    for idx, img in enumerate(images):
        result_img.paste(img,(0,y_offset))
        y_offset += img.size[1]

        progress = (idx+1) / len(images) * 100 #실제 퍼센트 정도 계산
        p_var.set(progress)
        progress_bar.update()
    
    dest_path = os.path.join(txt_dest_path.get(), "SILVER_PHOTO.jpg")
